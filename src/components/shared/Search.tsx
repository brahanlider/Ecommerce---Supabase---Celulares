...
export const Search = () => {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    // trim => quita al princio y final espacios
    if (searchTerm.trim()) {
  ...
  return (
    <>
      <div className="py-5 px-7 flex gap-10 items-center border-b border-slate-200">
        <form className="flex gap3 items-center flex-1" onSubmit={handleSearch}>
      ...
      <div className="p-5">
        {searchResults.length > 0 ? (
          ...
          <p className="text-sm text-gray-600">Nose encontraron resultados</p>
        )}
      </div>
    </>
  );
