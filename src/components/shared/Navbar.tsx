...
export const Navbar = () => {
  ...
  return (
    // fixed lo ponde yo QUITAR fixed h-24 z-50 w-full
    <header className=" bg-white text-black py-4 flex items-center justify-between px-5 border-b border-slate-200 lg:px-12">
      ...
      <nav className="space-x-5 hidden md:flex">
        {navbarLinks.map((link) => (
          <NavLink
            ...
            hover:text-cyan-800 hover:underline`
            }
          >
            {link.title}
          </NavLink>
      ...
      <div className="flex gap-5 items-center">
        ...
        <button className="relative" onClick={() => openSheet("cart")}>
          <span
            className="absolute -bottom-2 -right-2 w-5 h-5 grid place-items-center bg-black
